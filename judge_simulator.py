...
# Your API key is loaded from .env or the shell environment.
...
LLM_USAGE_FILE = Path(os.getenv("LLM_USAGE_FILE", str(DATASET_DIR.parent / ".llm_usage.json")))
RUN_LLM_PROBE = False  # Never spend a quota request on a connectivity probe.
...
class GeminiProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        query_params = urlencode({"key": self.api_key})
        req = urlrequest.Request(f"{base_url}?{query_params}", data=body, headers={"Content-Type": "application/json"})
        resp = urlrequest.urlopen(req, timeout=TIMEOUT_LLM)
        data = json.loads(resp.read().decode("utf-8"))
...
RESPOND ONLY WITH THIS EXACT JSON FORMAT:
{
  ...
  "hint": "<one sentence guidance for improvement, cryptic not direct>"
    ...
    def _record_daily_usage(self) -> None:
        today = datetime.utcnow().date().isoformat()
        try:
            data = {}
            ...
            LLM_USAGE_FILE.write_text(json.dumps(data), encoding="utf-8")
        except (OSError, ValueError, TypeError, json.JSONDecodeError) as exc:
            print_warn(f"Could not persist LLM usage counter: {exc}")

    def _wait_for_quota(self) -> None:
        """Serialize requests under Gemini's RPM limit."""
        if self.requests_used >= LLM_MAX_REQUESTS_PER_RUN:
            raise RuntimeError("per-run LLM request budget exhausted; using offline fallback")
        if self.daily_usage >= LLM_DAILY_REQUEST_LIMIT:
            raise RuntimeError("daily LLM request budget exhausted; using offline fallback")
        if LLM_MIN_INTERVAL_SECONDS <= 0:
            self.requests_used += 1
            self.daily_usage += 1
            self._record_daily_usage()
            return
        elapsed = time.monotonic() - self.last_request_at
        remaining = LLM_MIN_INTERVAL_SECONDS - elapsed
        if remaining > 0:
            print_info(f"Rate-limit pacing: waiting {remaining:.1f}s")
...
Score each dimension 0-10 with clear reasoning. Be STRICT."""
        ...
        try:
            ...
            return self._fallback_score(action, category, merchant, trigger, customer)
    ...
    def _fallback_score(self, action: Dict, category: Dict = None,
                        merchant: Dict = None, trigger: Dict = None,
                        customer: Dict = None) -> ScoreResult:
        """Useful offline score when the remote judge is unavailable."""
        body = action.get("body", "").lower()
        category = category or {}
        merchant = merchant or {}
        trigger = trigger or {}
        nums = len(re.findall(r'\d+(?:\.\d+)?', body))
        facts = nums + sum(1 for token in [
            merchant.get("identity", {}).get("name", ""),
            merchant.get("identity", {}).get("locality", ""),
            trigger.get("kind", ""),
        ] if str(token).lower() in body)
        category_terms = {
            "dentists": ["clinical", "patient", "dental", "aligner", "cleaning"],
            "salons": ["salon", "hair", "spa", "bridal", "service"],
            "restaurants": ["operator", "covers", "footfall", "pizza", "offer"],
            "gyms": ["coach", "trial", "member", "fitness", "yoga"],
            "pharmacies": ["pharmacist", "molecule", "batch", "medicine", "refill"],
        }
        terms = category_terms.get(category.get("slug", ""), [])
        category_hits = sum(term in body for term in terms)
        has_cta = bool(action.get("cta")) and any(x in body for x in ["reply", "want me", "shall", "can i", "confirm"])
        specificity = min(10, 5 + min(3, facts // 2))
        category_fit = min(10, 5 + min(5, category_hits))
        merchant_fit = min(10, 5 + min(5, sum(x in body for x in [
            str(merchant.get("identity", {}).get("owner_first_name", "")).lower(),
            str(merchant.get("identity", {}).get("locality", "")).lower(),
            "snapshot", "account signal", "last mentioned",
        ] if x)))
        decision_quality = min(10, 6 + (1 if trigger.get("kind", "") in body else 0) + (1 if has_cta else 0))
        engagement = min(10, 6 + (2 if has_cta else 0) + (1 if "want" in body or "reply" in body else 0))
        return ScoreResult(
            specificity=specificity,
            specificity_reason="Offline estimate based on checkable numbers and named context.",
            category_fit=category_fit,
            category_fit_reason="Offline estimate based on category-native vocabulary.",
            merchant_fit=merchant_fit,
            merchant_fit_reason="Offline estimate based on owner, locality, and account evidence.",
            decision_quality=decision_quality,
            decision_quality_reason="Offline estimate based on trigger relevance and a clear next step.",
            engagement_compulsion=engagement,
            engagement_reason="Offline estimate based on a focused CTA and reply invitation.",
            hint="Remote judge unavailable; paced requests or switch to a local provider."
        )
...
def main():
    ...
    # Do not make a probe request. Gemini free tiers count it against RPD, and
    # the first scoring request already verifies connectivity.
    print_info("LLM probe disabled to preserve the provider request budget")
